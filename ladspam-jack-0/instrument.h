...
namespace ladspam_jack
{
	struct instrument : ladspam_jack::synth
	{
		...
		virtual int process(jack_nframes_t nframes)
		{
			...
			unsigned current_sample_in_chunks = 0;
			...
			for (unsigned frame_index = 0; frame_index < nframes; ++frame_index)
			{
				...
				for (jack_nframes_t frame = 0, max = nframes; frame < max; ++frame) 
				{
					...
					{
						{
							ladspam::synth::buffer &buffer = *m_voices[oldest_voice(frame_index)]->m_port_buffers[1];
							buffer[frame_index % m_control_period] = m_voices[voice_index]->m_gate;
						}
						
						{
							ladspam::synth::buffer &buffer = *m_voices[oldest_voice(frame_index)]->m_port_buffers[2];
							buffer[frame_index % m_control_period] =  m_voices[voice_index]->m_on_velocity / 128.0;
						}
						
						{
							ladspam::synth::buffer &buffer = *m_voices[oldest_voice(frame_index)]->m_port_buffers[3];
							buffer[frame_index % m_control_period] =  note_frequency(m_voices[voice_index]->m_note);
						}
						
						{
							ladspam::synth::buffer &buffer = *m_voices[oldest_voice(frame_index)]->m_port_buffers[4];
							buffer[frame_index % m_control_period] = note_frequency(m_voices[voice_index]->m_note) / (float)jack_get_sample_rate(m_jack_client);
						}
					}
				}
				
				
				if (0 == frame_index % m_control_period)
				{
